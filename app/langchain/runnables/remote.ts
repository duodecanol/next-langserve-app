import {Runnable, RunnableBatchOptions, getCallbackManagerForConfig, type RunnableConfig} from "@langchain/core/runnables"
...
import { IterableReadableStream, concat } from "@langchain/core/utils/stream";
  ...
  if (typeof obj === "object") {
    ...
    if (isSuperset(keys, new Set(["content", "type", "additional_kwargs"]))) {
      ...
      if (obj.type === "HumanMessageChunk") {
        return new HumanMessageChunk(obj);
      }
      if (obj.type === "SystemMessageChunk") {
        return new SystemMessageChunk(obj);
      }
      if (obj.type === "ChatMessageChunk") {
        return new ChatMessageChunk(obj);
      }
